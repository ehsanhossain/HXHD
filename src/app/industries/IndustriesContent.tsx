...
export function IndustriesContent() {
  ...
  const sectors = INDUSTRIES.map((ind, i) => {
    ...
    return { ...ind, ...c.industries.items[i], count: products.length };
  });
      ...
      <section className="section">
        <div className="shell">
          <Reveal className="max-w-3xl mb-14">
            <p className="eyebrow mb-5">{c.industries.sectorsEyebrow}</p>
            <h2 className="text-step-3 mb-5">{c.industries.sectorsTitle}</h2>
            <p className="text-[var(--ink-3)] text-step-0 leading-relaxed">
              {c.industries.sectorsLead}
            </p>
          </Reveal>
                  ...
                  <div className="flex flex-col flex-grow p-7">
                    ...
                    <div className="mt-auto flex items-center justify-between gap-3 pt-5 border-t border-[var(--line)]">
                      <span className="text-xs font-bold uppercase tracking-[0.1em] text-[var(--ink-3)]">
                        {fill(s.count === 1 ? c.industries.productCount : c.industries.productCountPlural, { n: s.count })}
                      </span>
        ...
        <div className="shell relative max-w-4xl">
          <Reveal>
            ...
            <p className="text-step-1 text-white/70 font-light leading-relaxed mb-10">
              {fill(c.industries.whyBody, { zone: c.zone })}
            </p>
          </Reveal>

          <Stagger className="grid grid-cols-1 sm:grid-cols-3 gap-px bg-white/10 border border-white/10">
            {[
              { k: BANGLADESH.investmentLabel, v: c.industries.statInvestment },
              { k: `${BANGLADESH.jobs}+`, v: c.industries.statJobs },
              { k: `${PRODUCTS.length}`, v: c.industries.statCatalogue },
            ].map((s) => (
              <StaggerItem key={s.v}>
